...
|-------|---------|---------------|----------|
|Mantis|10.129.244.187|Windows|Hard|
...
└─$ nmap -sC -sV -p- -Pn  --min-rate 10000 -oA nmap_report 10.129.244.187
Starting Nmap 7.94SVN ( https://nmap.org ) at 2024-12-06 20:58 IST
Warning: 10.129.244.187 giving up on port because retransmission cap hit (10).
Nmap scan report for 10.129.244.187
...
```
10.129.244.187	htb.local
10.129.244.187	mantis.htb.local
```
...
SMB         10.129.244.187  445    MANTIS           [*] Windows Server 2008 R2 Standard 7601 Service Pack 1 x64 (name:MANTIS) (domain:htb.local) (signing:True) (SMBv1:True)
SMB         10.129.244.187  445    MANTIS           [-] htb.local\guest: STATUS_ACCOUNT_DISABLED 
                                                                                                                                                      
┌──(frodo㉿kali)-[~/hack-the-box/mantis]
└─$ netexec smb mantis.htb.local -u '' -p '' --shares
SMB         10.129.244.187  445    MANTIS           [*] Windows Server 2008 R2 Standard 7601 Service Pack 1 x64 (name:MANTIS) (domain:htb.local) (signing:True) (SMBv1:True)
SMB         10.129.244.187  445    MANTIS           [+] htb.local\: 
SMB         10.129.244.187  445    MANTIS           [-] Error enumerating shares: STATUS_ACCESS_DENIED
                                                                                                                                                        
┌──(frodo㉿kali)-[~/hack-the-box/mantis]
└─$ netexec smb mantis.htb.local -u anonymous -p '' --shares
SMB         10.129.244.187  445    MANTIS           [*] Windows Server 2008 R2 Standard 7601 Service Pack 1 x64 (name:MANTIS) (domain:htb.local) (signing:True) (SMBv1:True)
SMB         10.129.244.187  445    MANTIS           [-] htb.local\anonymous: STATUS_LOGON_FAILURE
```
...
SMB         10.129.244.187  445    MANTIS           [*] Windows Server 2008 R2 Standard 7601 Service Pack 1 x64 (name:MANTIS) (domain:htb.local) (signing:True) (SMBv1:True)
LDAP        10.129.244.187  389    MANTIS           [-] htb.local\guest: STATUS_ACCOUNT_DISABLED
                                                                                                                                                        
┌──(frodo㉿kali)-[~/hack-the-box/mantis]
└─$ netexec ldap mantis.htb.local -u '' -p ''               
SMB         10.129.244.187  445    MANTIS           [*] Windows Server 2008 R2 Standard 7601 Service Pack 1 x64 (name:MANTIS) (domain:htb.local) (signing:True) (SMBv1:True)
LDAP        10.129.244.187  389    MANTIS           [-] Error in searchRequest -> operationsError: 000004DC: LdapErr: DSID-0C09075A, comment: In order to perform this operation a successful bind must be completed on the connection., data 0, v1db1
LDAP        10.129.244.187  389    MANTIS           [+] htb.local\: 
```
...
```shell                   
┌──(frodo㉿kali)-[~/hack-the-box/mantis]
└─$ gobuster dir -u http://10.129.244.187:1337 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -t 40 -o dir_bruteforce_1337 
===============================================================
Gobuster v3.6
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@firefart)
===============================================================
[+] Url:                     http://10.129.244.187:1337
[+] Method:                  GET
[+] Threads:                 40
...
Error: error on running gobuster: unable to connect to http://10.129.244.187:1337/: Get "http://10.129.244.187:1337/": dial tcp 10.129.244.187:1337: connect: no route to host
```
...
After resetting the machine, I started directory bruteforcing on port 1337 again.
...
/secure_notes         (Status: 301) [Size: 163] [--> http://10.129.244.187:1337/secure_notes/]
