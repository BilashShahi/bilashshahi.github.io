---
title: Vintage
description: In this walkthrough, I am going to demonstrate how I was able to pawn a level hard windows machine called 'Vintage' in hack-the-box.
categories: [Walkthroughs, HackTheBox]
tags: [windows,Active Directory,Vintage,htb,hackthebox,ldap,dpapi.py,impacket-dpapi,getTGT.py,impacket-getTGT,impacket-getST,RBCD]
---
...
Initial credentials have been provided with the box as follows:
...
To proceed, we will retry the authentication attempt, but this time using **Kerberos**.
...
We are going to spray **p.rosa's** on all the users. It's possible that the same password is used across multiple accounts in the domain.
...
Now that we have got our foothold in the target, we are going to 


### DPAPI

1. Master Key - "$env:appdata\Microsoft\Protect\S-1-5-21-4024337825-2033394866-2055507597-1115"
    1. 4dbf04d8-529b-4b4c-b4ae-8e875e4fe847
    2. 99cf41a3-a552-4cf7-a8d7-aca2d6f7339b

"$env:LOCALAPPDATA\Microsoft\Credentials"

Compress-Archive -Path "$env:appdata\Microsoft\Protect\S-1-5-21-4024337825-2033394866-2055507597-1115" -DestinationPath "$env:USERPROFILE\Documents\masterkey.zip"

```shell

```

2. Credentials - "$env:appdata\Microsoft\Credentials"
...
Using **c.neri** add an SPN for **svc_sql**
```shell
┌──(frodo㉿kali)-[~/hack-the-box/vintage]
└─$ impacket-getTGT vintage.htb/c.neri:'Zer0the0ne'                 
Impacket v0.12.0 - Copyright Fortra, LLC and its affiliated companies 
...
Most likely the **svc_sql** account is disabled already.
